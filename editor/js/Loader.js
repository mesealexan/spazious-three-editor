		...
		switch (extension) {
				...
				reader.addEventListener('load', async function (event) {
					...
					loader.parse(contents, '', function (result) {
						...
						scene.traverse((child) => {
							if (child.material) {
								if (child.material.aoMap) child.material.lightMap = child.material.aoMap;
							}
						})
						scene.name = filename;
			...
			case 'hdr':
				...
				reader.addEventListener('load', async function (event) {
					...
					var { RGBELoader } = await import('../../examples/jsm/loaders/RGBELoader.js');

					new RGBELoader().load(contents, function (result) {
